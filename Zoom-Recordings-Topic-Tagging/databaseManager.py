...
# Insert ['meetingId', 'transcript', 'filtered_transcript', 'chat', 'filtered_chat', 'meetingDetail',
#                 'registrantsInfo', 'registrantsCount', 'regisQuestion', 'participantsInfo', 'participantsCount']
# into database (Local)

# Need to have summary.txt, transcript, chat files downloaded with main.py under same directory

def correctIllegalChar(filename):
    ret = filename.lower()
    illegalFirstChars = [' ', '.', '_', '-']
    illegalChars = [ '?', '"', "'"] # illegal chars for sql
...
def insertTrChat():
    """Iterate through the list of summary.txt, get transcript, chat files and insert them into database"""
    remainingIDs = zoomRecordingMetadata.getAllMeetingId()
    for i in range(len(remainingIDs)):
        if '/' in remainingIDs[i]:
            remainingIDs[i].replace('/', '%2F') # '/' char exist in the meetingId, must be changed to %2F when making query to zoom api
            continue
        remainingIDs[i] = correctIllegalChar(remainingIDs[i])

    # insert to row to database that has transcript, chat
...
def insertAWSTranscript():
    """Insert AWS transcribed transcripts to database"""
