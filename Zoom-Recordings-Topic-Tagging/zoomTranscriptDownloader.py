...
for i in range(len(users)):
    ...
    currentTo = datetime.date.today()   # looks for recordings [today(), (2014,1,1)]
    ...
    while currentFrom >= firstDate:
        ...
        while True:
            ...
            if "error" in videolistjson:
                if videolistjson["error"]["code"] == 403:
                    ...
                    sys.exit(
                        "Unknown Error. Please run program again")
            break
        ...
        if (len(currUserMeetings) > 0):
            ...
            # for loop iterates through a meeting object, and we specifically want to construct the meeting name
